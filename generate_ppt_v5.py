...
# -*- coding: utf-8 -*-
"""
日本跨境电商 ERP 系统 - 合作伙伴版演示文档（v5 完整显示版）
调整布局，确保每页内容完整显示
生成日期：2026-03-07
"""
...
def create_final_ppt():
    ...
    def add_content_slide(title, content_lines, slide_num, total):
        ...
        # 标题
        title_box = slide.shapes.add_textbox(
            Inches(0.6), Inches(0.3), Inches(11.5), Inches(0.6)
        )
        tf = title_box.text_frame
        p = tf.paragraphs[0]
        p.text = title
        ...
        content_box = slide.shapes.add_textbox(
            ...
            prs.slide_width - Inches(1.2), prs.slide_height - Inches(1.5)
        )
        ...
        page_num = slide.shapes.add_textbox(
            prs.slide_width - Inches(1), prs.slide_height - Inches(0.35),
    ...
    def add_two_column_slide(title, left_title, left_items, right_title, right_items, slide_num, total):
        """双列布局页"""
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        ...
        left_box = slide.shapes.add_textbox(
            Inches(0.6), Inches(1.0), Inches(5.8), prs.slide_height - Inches(1.5)
        )
        ...
        right_box = slide.shapes.add_textbox(
            Inches(6.8), Inches(1.0), Inches(5.8), prs.slide_height - Inches(1.5)
        )
    ...
    total_slides = 16  # 增加到 16 页以确保内容完整
    ...
    add_content_slide("四、商业价值", [
        "💰 客户收益（年销 5000 万日元卖家）",
        "",
        "效率提升：订单 10 倍 | 上新 6 倍 | 财务 24 倍 | 客服 12 倍",
        "",
        "成本降低：人工 -50% | 物流 -15% | 库存 -30% | 错误 -90%",
        "",
        "收入增长：店铺 3→10 个 | 转化率 +15% | ROI +20%",
        "",
        "📊 投资回报率：44 倍！",
    ...
    # 第 9 页：投入与回报（开发投入）
    add_content_slide("五、投入与回报 - 开发投入", [
        "🛠️ 开发投入：¥165 万",
        "",
        "人力成本：¥150 万",
        "• 后端开发：3 人 × 3 个月",
    ...
    # 第 10 页：投入与回报（收益预测）
    add_content_slide("五、投入与回报 - 收益预测", [
        "📈 收益预测（SaaS 订阅模式）",
        "",
        "基础版：¥5 万/年",
        "• 目标客户：小卖家",
    ...
    tf = title_box.text_frame
    p = tf.paragraphs[0]
    p.text = "📞 联系我们\n\n随时沟通，共创双赢！\n\n制作人：二狗子 🐕\n版本：5.0 完整显示版 | 2026 年 3 月 7 日"
    p.font.size = Pt(24)
    p.font.color.rgb = DARK
    
    # 保存文件
    output_path = "/home/admin/.openclaw/workspace/日本跨境电商 ERP 系统_合作伙伴版_v5_完整版.pptx"
    prs.save(output_path)
    print(f"✅ 完整版 PPT 已生成：{output_path}")
    return output_path
